(** * Basics: Functional Programming in Coq - TRAINING VERSION *)

(* 
   ###############################################################
   ###  THIS IS A TRAINING EXERCISE FILE                      ###
   ###  Complete the definitions and proofs marked with       ###
   ###  "EXERCISE" and "FILL IN HERE"                         ###
   ###  Replace "Admitted" with your solutions                ###
   ###############################################################
*)
...
(** 
    EXERCISE: Complete the definition of [next_working_day].
    This function should take a day and return the next working day.
    - For Monday through Thursday, return the next day
    - For Friday, Saturday, and Sunday, return Monday
    Use pattern matching with [match d with] syntax.
*)
...
(** 
    EXERCISE: Complete this example assertion and its proof.
    The assertion states that the second working day after Saturday is Tuesday.
    Once you have implemented [next_working_day] correctly, prove this with:
    1. [simpl] to simplify the goal
    2. [reflexivity] to complete the proof
*)
...
(** 
    EXERCISE: Define boolean negation [negb].
    This function should return the opposite boolean value.
    - [negb true] should be [false]
    - [negb false] should be [true]
*)
...
(** 
    EXERCISE: Define boolean AND [andb].
    This function should return [true] only when both inputs are [true].
    - [andb true true] should be [true]
    - [andb true false] should be [false] 
    - [andb false true] should be [false]
    - [andb false false] should be [false]
*)
...
(** 
    EXERCISE: Define boolean OR [orb].
    This function should return [true] when at least one input is [true].
    - [orb true true] should be [true]
    - [orb true false] should be [true]
    - [orb false true] should be [true]
    - [orb false false] should be [false]
*)
...
(** 
    EXERCISE: Complete these test proofs for the [orb] function.
    Each proof should use [simpl] followed by [reflexivity].
*)
...
(** 
    EXERCISE: Complete this test using the infix notation.
    Use [simpl] followed by [reflexivity].
*)
...
(** 
    EXERCISE: Redefine the boolean operations using conditional expressions.
    Coq supports [if-then-else] syntax. Use this to reimplement:
    - [negb'] - boolean negation using conditionals
    - [andb'] - boolean AND using conditionals  
    - [orb'] - boolean OR using conditionals
*)
Definition negb' (b:bool) : bool :=
  if b then false
  else true.
...
(** 
    EXERCISE: Define [invert] function for black/white values.
    This function should use conditional syntax to:
    - Return [bw_white] if input is [bw_black]
    - Return [bw_black] if input is [bw_white]
    Note: The first constructor ([bw_black]) is considered "true" in conditionals.
*)
...
(** 
    EXERCISE: Define [monochrome] function.
    This function should return [true] for [black] and [white] colors,
    and [false] for any [primary] color.
    Use pattern matching with [match c with] syntax.
*)
...
(** 
    EXERCISE: Define [isred] function.
    This function should return [true] only for [primary red],
    and [false] for all other colors.
    Use pattern matching. For [primary] colors that aren't red,
    you can use the wildcard pattern [_].
*)
...
(** 
    EXERCISE: Define [all_zero] function for nybbles.
    This function should return [true] if all four bits are [B0],
    and [false] otherwise.
    Use pattern matching on the [bits] constructor.
    You can use wildcard patterns [_] for the second case.
*)
...
(** 
    EXERCISE: Define the predecessor function [pred].
    This function should return the predecessor of a natural number:
    - [pred O] should be [O] (0 has no predecessor)
    - [pred (S n')] should be [n'] (the predecessor of n+1 is n)
    Use pattern matching on the structure of natural numbers.
*)
...
(** 
    EXERCISE: Define [minustwo] function.
    This function should subtract 2 from a natural number:
    - [minustwo O] should be [O]
    - [minustwo (S O)] should be [O] (1 - 2 = 0, since we can't go below 0)
    - [minustwo (S (S n'))] should be [n'] (subtract 2)
    Use pattern matching with nested [S] constructors.
*)
...
(** 
    EXERCISE: Define recursive [even] function.
    This function should determine if a natural number is even:
    - [even O] should be [true] (0 is even)
    - [even (S O)] should be [false] (1 is odd)
    - [even (S (S n'))] should be [even n'] (recursively check n-2)
    Use [Fixpoint] for recursion and pattern matching.
*)
...
(** 
    EXERCISE: Define [odd] function using [even].
    This should return the opposite of [even n].
    Hint: Use the [negb] function you defined earlier.
*)
...
(** 
    EXERCISE: Define recursive addition [plus].
    This function should add two natural numbers:
    - [plus O m] should be [m] (0 + m = m)
    - [plus (S n') m] should be [S (plus n' m)] (recursive case)
    Use [Fixpoint] and pattern matching on the first argument.
*)
...
(** 
    EXERCISE: Define recursive multiplication [mult].
    This function should multiply two natural numbers:
    - [mult O m] should be [O] (0 * m = 0)
    - [mult (S n') m] should be [plus m (mult n' m)] (n * m = m + (n-1) * m)
    Use [Fixpoint], pattern matching, and the [plus] function.
*)
...
(** 
    EXERCISE: Define recursive subtraction [minus].
    This function should subtract the second argument from the first:
    - [minus O _] should be [O] (0 - anything = 0)
    - [minus (S _) O] should be [n] (n - 0 = n)
    - [minus (S n') (S m')] should be [minus n' m'] (recursive case)
    Use [Fixpoint] and simultaneous pattern matching with comma.
*)
...
(** 
    EXERCISE: Define recursive exponentiation [exp].
    This function should raise base to the power of the second argument:
    - [exp base O] should be [S O] (anything^0 = 1)
    - [exp base (S p)] should be [mult base (exp base p)] (base^(p+1) = base * base^p)
    Use [Fixpoint], pattern matching on power, and the [mult] function.
*)
...
(** 
    EXERCISE: Define recursive equality test [eqb].
    This function should test if two natural numbers are equal:
    - [eqb O O] should be [true]
    - [eqb O (S m')] should be [false]
    - [eqb (S n') O] should be [false]
    - [eqb (S n') (S m')] should be [eqb n' m'] (recursive case)
...
(** 
    EXERCISE: Define recursive less-than-or-equal test [leb].
    This function should test if the first number is ≤ the second:
    - [leb O m] should be [true] (0 ≤ anything)
    - [leb (S n') O] should be [false] (positive number > 0)
    - [leb (S n') (S m')] should be [leb n' m'] (recursive case)
    Use [Fixpoint] and nested pattern matching.
*)
...
(** 
    EXERCISE: Prove that 0 is a left identity for addition.
    This theorem states that adding 0 to any number n gives n.
    Steps: 1) [intros n] to introduce the variable
           2) [simpl] to simplify (0 + n reduces to n)  
           3) [reflexivity] to finish the proof
*)
...
(** 
    EXERCISE: Prove that 1 is a left successor operation for addition.
    This theorem states that 1 + n = S n (the successor of n).
*)
...
(** 
    ...
    This theorem states that 0 * n = 0 for any n.
*)
...
(** 
    EXERCISE: Complete this proof using rewriting.
    This theorem states that if n = m, then n + n = m + m.
    Steps: 1) [intros n m] to introduce variables
           2) [intros H] to introduce the hypothesis n = m  
           3) [rewrite -> H] to replace n with m using the hypothesis
...
(** 
    EXERCISE: Prove using previously established theorems.
    This theorem uses the fact that n * 0 = 0 (from [mult_n_O]).
    Steps: 1) [intros p q] to introduce variables
           2) [rewrite <- mult_n_O] twice to simplify p * 0 and q * 0 to 0
           3) [reflexivity] to finish (0 + 0 = 0)
*)
...
(** 
    EXERCISE: Prove using case analysis (destruct).
    This theorem states that n + 1 is never equal to 0.
    Steps: 1) [intros n] to introduce the variable
           2) [destruct n as [| n'] eqn:E] to consider cases n = 0 and n = S n'
           3) In each case, use [reflexivity] to show the goal is true
    Use bullets [-] to separate the cases.
*)
...
(** 
    EXERCISE: Prove that boolean negation is involutive.
    This means that negating a boolean twice gives the original value.
    Steps: 1) [intros b] to introduce the boolean variable
           2) [destruct b eqn:E] to consider cases b = true and b = false
           3) In each case, use [reflexivity] to verify the equality
*)
